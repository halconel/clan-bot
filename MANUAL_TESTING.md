...
4. **Валидация никнейма (Kingdom Clash правила: любые символы, 1-15 символов):**
   - `/register` → отправьте "AB" → ✅ должен принять (короткие ники разрешены)
   - `/register` → отправьте "Test@Player!" → ✅ должен принять (спецсимволы разрешены)
   - `/register` → отправьте "ThisNicknameIsTooLongForGame" → ❌ должна быть ошибка "слишком длинный"
